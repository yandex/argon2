...
#define ROUND(r) \
    ...
    template<>
    void Blake2B<InstructionSet::SSE41>::initial_xor__(uint8_t *h, const uint8_t *p) {
    ...
    template<>
    void Blake2B<InstructionSet::SSE41>::compress__(const uint8_t block[BLAKE2B_BLOCKBYTES]) {
        static const __m128i* iv = get_iv();
